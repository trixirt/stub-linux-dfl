...
static void n5010_hssi_update_link(struct net_device *netdev)
{
	...
	if (link_status != npriv->link_status) {
		...
		dev_info(&netdev->dev, "link state: %u\n", link_status);

		if (link_status == FEC_RX_STATUS_LINK)
...
static void poll_work(struct work_struct *arg)
{
	...
	priv = container_of(arg, struct n5010_hssi_drvdata, poll_workq);

	for (port = 0; port < priv->port_cnt; port++)
		n5010_hssi_update_link(priv->netdev[port]);
}

static void poll_timerf(struct timer_list *timer_arg)
{
	struct n5010_hssi_drvdata *priv = from_timer(priv, timer_arg, poll_timer);
...
static int netdev_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	regmap_write(npriv->regmap_mac, MAC_RX_MTU, new_mtu);
...
static int netdev_set_mac_address(struct net_device *netdev, void *p)
{
	...
	regmap_write(npriv->regmap_mac, MAC_TX_SRC_ADDR_HI, mac_part1);
	regmap_write(npriv->regmap_mac, MAC_TX_SRC_ADDR_LO, mac_part2);
...
static struct stat_info stats_100g[] = {
	...
	{STAT_INFO(0x802, "tx_jabbers HUU")},
	...
	{STAT_INFO(0x902, "rx_jabbers HAA")},
...
{
	void *base = priv->base + port * MB_PORT_SIZE;
...
static int n5010_hssi_create_netdev(struct n5010_hssi_drvdata *priv, u64 port)
{
	struct device *dev = &priv->dfl_dev->dev;
...
static int n5010_hssi_probe(struct dfl_device *dfl_dev)
{
	...
	for (port = 0; port < priv->port_cnt; port++) {
		ret = n5010_hssi_create_netdev(priv, port);
		if (ret)
			break;
	}
...
static int n5010_hssi_remove(struct dfl_device *dfl_dev)
{
